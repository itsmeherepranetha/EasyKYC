...
st.title("Welcome to the EasyKYC")
...
def main_content(image_file, face_image_file, conn):
    if image_file is not None:
        ...
        if face_image is not None:
            ...
            face_image_path2 = detect_and_extract_face(img=image_roi.copy())
...
def main():
    ...
    image_file = st.file_uploader("Upload PAN Card",type=["jpg", "png", "jpeg"])  #  file uploader widget in the Streamlit app.

    if image_file is not None:
        face_image_file = st.file_uploader("Upload Face Image/photo",type=["jpg", "png", "jpeg"]) #  file uploader widget in the Streamlit app.
